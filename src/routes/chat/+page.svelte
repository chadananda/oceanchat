...
<ChatLayout {conversations} {currentRepository}>
 <div slot="chat-messages" class="chat-messages">
   ...
   {#each messages as message}
     <div class={`chat-message ${message.sender === 'user' ? 'text-right' : 'text-left'}`}>
       <div class={`inline-block p-3 rounded-lg ${message.sender === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-800 text-white'}`}>
         {message.content}
       </div>
     </div>
   {/each}
 ...
 <form slot="chat-input" class="chat-input-form flex items-center" on:submit|preventDefault={handleSubmit}>
   <input
